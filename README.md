...
`search_codes.py` and `search_terms.py`, entry point `download_patent_ids` in both.

This stage collects family identifiers only. Each output row is `Patent_ID`, `Family_ID`, `Country`.

### Method 1: classification codes (`search_codes.py`)
...
### Method 2: keywords (`search_terms.py`)
...
`search_terms.py`, eight entries, anything else scoring 99:

| Score | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|---|---|---|---|---|---|---|---|---|
| Office | EP | US | WO | GB | AU | CA | NZ | IE |

`search_codes.py`, 35 entries: EP (1), US (2), WO (3), CA (6), then the EPC national offices alphabetically (AT 7 through SK 30), then GB (31), IE (32), CH (33), AU (34), NZ (35).
...
`search_terms.py` has the identical signature. Both return the deduplicated DataFrame and write the CSV at the same time.
