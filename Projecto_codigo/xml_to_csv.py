...
# NOTE: llama-3.1-8b-instant is deprecated by Groq (decommission 2026-08-16) but
# still served, so kept for now. The gpt-oss replacement is a reasoning model and
# returned empty content under our small max_tokens (reasoning ate the budget);
# migrating to it will need reasoning_effort="low" + a much larger max_tokens.
...
# NOTE: llama-3.3-70b-versatile is likewise deprecated (decommission 2026-08-16)
# but still served; kept until we do the gpt-oss migration properly.
...
def _section_label_from_thead(thead, ncols: int) -> str:
    """If a <thead> contains a SHORT full-width spanning row that reads like a
    section divider (e.g. 'HeLa day 3'), return its text; otherwise ''.

    This distinguishes a section divider from a long full-width CAPTION. A divider
    is short (<= 40 chars, <= 6 words) and is not a sentence (no trailing period);
    a caption like 'The experiments were conducted using ... Table 3.' is long and
    ends in a period, so it stays an annotation. This only *reports* a candidate
    label — it is used to build a 'section' column solely when >= 2 distinct labels
    appear across a table's tgroups (see the guarded call site), so single-section
    and caption-only tables are never affected."""
    if thead is None:
        return ""
    for row_el in thead.findall("row"):
        if not _is_spanning_row(row_el, ncols):
            continue
...
def extract_tables(xml_file: str) -> list[dict]:
    ...
    for tables_el in root.iter("tables"):
        for table_el in tables_el.findall("table"):
            ...
            combined_row_sections: list[str]      = []   # per-data-row section label
            ...
            for tgroup in table_el.findall("tgroup"):
                ...
                # Candidate section label for THIS tgroup (e.g. 'HeLa day 3').
                # Only becomes a real column if >=2 distinct labels appear overall.
                section_label = _section_label_from_thead(thead, ncols)
                ...
                combined_row_sections.extend([section_label] * len(data_rows))

            if not combined_rows:
                continue

            # ── Multi-section preservation (guarded, additive) ───────────────
            # When one <table> stacks several LABELLED sections as separate
            # tgroups (e.g. 'HeLa day 3' / 'HeLa day 6' / 'Hep3B day 3' viability
            # screens), keep each label as a trailing 'section' column so the
            # condition (cell line / day) isn't lost to the context file.
            #
            # Guard: activate ONLY when >= 2 DISTINCT section labels were seen, so
            # ordinary single-section tables and caption-only tables are byte-for-
            # byte unchanged. The label is appended as the LAST column to BOTH the
            # header and EVERY data row, so the header-vs-data column-count
            # invariant (which the deterministic span-merge relies on downstream)
            # holds exactly — the header-repair passes are unaffected.
            distinct_sections = {s for s in combined_row_sections if s}
            if (len(combined_row_sections) == len(combined_rows)
                    and len(distinct_sections) >= 2):
                for row, sec in zip(combined_rows, combined_row_sections):
                    row.append(sec)
                if combined_headers:
                    for hrow in combined_headers[:-1]:
                        hrow.append("")
                    combined_headers[-1].append("section")
                else:
                    combined_headers = [["section"]]
