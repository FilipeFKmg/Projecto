...
prepare a clean ID list for Phase 2 (epo_api_Metadata.py).
...
Search strategy:
    ...
    - Family deduplication: across all queries and all years, only one
      representative per patent family is kept. The preferred jurisdiction
      order is EP first, then US, then WO, then the rest
      (EP > US > WO > GB > AU > CA > NZ > IE).
...
#
# Lower score = higher priority. The chosen order is:
#       EP (1)  -> European Patent Office documents come first, because this
#                  dataset is built around EPO patents and the EP family
#                  member is the primary document of interest.
#       US (2)  -> United States documents come second; they are published in
#                  English, which keeps Phase 2 titles/abstracts readable.
#       WO (3)  -> PCT (World) publications come third; also English-friendly.
#       The remaining English-publishing jurisdictions (GB, AU, CA, NZ, IE)
#       follow, so that any title/abstract fetched in Phase 2 is still likely
#       to be available in English.
#
# Any country NOT listed here is assigned the fallback score 99 (see the
# COUNTRY_PRIORITY.get(country, 99) calls below), so unknown jurisdictions are
# only ever chosen when no preferred member exists in the family.
# =============================================================================
COUNTRY_PRIORITY = {
    'EP': 1, 'US': 2, 'WO': 3, 'GB': 4,
    'AU': 5, 'CA': 6, 'NZ': 7, 'IE': 8,
...
def _deduplicate_by_family(id_records: list, seen_families: dict = None) -> tuple:
    ...
    Selection rule: COUNTRY_PRIORITY (EP=1 > US=2 > WO=3 > ...). The lowest
    score wins, so EP documents are preferred first, then US, then WO, then the
    remaining English-publishing jurisdictions. Any country outside the table
    is treated as score 99 and only chosen as a last resort.
    ...
    representative — e.g. an EP grant published after a CN priority filing.
    ...
    for rec in id_records:
        ...
        # Lower score = higher priority (EP=1, US=2, WO=3, ...). Unknown
        # countries fall back to 99 so they are only chosen when nothing
        # better is available in the family.
...
def download_patent_ids(
    ...
    if only_applicant:
        if not applicant_filter:
            ...
            return pd.DataFrame()
        # Add a single empty item so the extraction loop runs exactly once
        independent_queries = [""]
    ...
    def search_patent_ids(cql_query: str, total_patents: int) -> list:
        ...
        while start_index <= total_patents:
            ...
            # A single-document page is returned as a dict, not a list, so we
            # normalise it into a one-element list for uniform handling.
    ...
    def search_with_slicing(base_cql: str, year: int) -> list:
        ...
        # Auxiliary helper that builds the correct CQL string depending on
        # which filters are active (applicant-only, terms + applicant, or
        # terms only), always appending the supplied date condition.
        ...
        # Build the yearly query first and count its results
    ...
    try:
        ...
        for current_year in range(start_year, end_year + 1):
            ...
            # in previous years (unless a better-priority representative is found)
        ...
        # Unknown countries get 99 so they sort to the bottom.
        final_df['Priority'] = final_df['Country'].map(COUNTRY_PRIORITY).fillna(99)

        # Sort so the highest priority (lowest number, i.e. EP) is at the top
        final_df = final_df.sort_values('Priority')

        # With the rows sorted by priority, keep='first' safely keeps the
        # highest-priority representative (EP > US > WO > ...) per family
